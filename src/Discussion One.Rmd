...
pieChartData <- data.frame(Genre = c("Battle Royale", "Collectible Card Game", "First-Person Shooter", "Multiplayer Online Battle Arena"),
                    ...
                    mutate(pieChartData, labels = round((TotalTournaments/total) * 100, 1))
...
mutate(prop = percent(TotalTournaments / sum(TotalTournaments))) -> mydf 

pie <- ggplot(mydf, aes(x = "", y = TotalTournaments, fill = fct_inorder(Genre))) +
       geom_bar(width = 1, stat = "identity") +
       coord_polar("y", start = 0) +
